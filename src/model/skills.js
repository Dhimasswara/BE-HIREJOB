...
const selectAllSkills = (limit, offset, searchParam, sortBY, sort) => {
  return Pool.query(`SELECT * FROM skills WHERE name_skill LIKE '%${searchParam}%' ORDER BY ${sortBY} ${sort} LIMIT ${limit} OFFSET ${offset} `);
}
...
const insertSkills = (data) => {
  ...
  return Pool.query(`INSERT INTO skills (id_skill,name_skill,id_worker) VALUES('${id}','${name}','${id_worker}')`);
}

const updateSkills = (data) =>{
    const { id,name,id_worker} = data;
    return Pool.query(`UPDATE skills SET name_skill='${name}' WHERE id_worker='${id_worker}' and id_skill='${id}'`);
}
...
const findName = (name, id_worker) => {
  return new Promise((resolve, reject) =>
    Pool.query(`SELECT name_skill FROM skills where id_worker='${id_worker}' and name_skill ILIKE '%${name}%'`, (error, result) => {
      if (!error) {
        resolve(result)
      } else {
        reject(error)
